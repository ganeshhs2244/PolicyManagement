...
public class PolicyMapper implements FieldSetMapper<LicPolicy> {
    ...
    public LicPolicy mapFieldSet(FieldSet fieldSet) throws BindException {
        ...
        try {
            policy.setFamilyCode(fieldSet.readString("familyCode"));
            policy.setPolicyHolderName(fieldSet.readString("policyHolderName"));
            policy.setDOB(parseDate(fieldSet.readString("DOB")));
            policy.setMobile(fieldSet.readString("mobile"));
            policy.setEmail(fieldSet.readString("email"));
            policy.setAddress(fieldSet.readString("address"));
            policy.setPolicyNumber(fieldSet.readString("policyNumber"));
            policy.setAgencyCode(fieldSet.readString("agencyCode"));
            policy.setCommecementDate(parseDate(fieldSet.readString("commecementDate")));
            policy.setPlan(fieldSet.readString("plan"));
            policy.setTerm(fieldSet.readString("Term"));
            policy.setPPT(fieldSet.readString("PPT"));
            policy.setSumAssured(fieldSet.readString("sumAssured"));
            policy.setMode(fieldSet.readString("mode"));
            policy.setFUPDate(parseDate(fieldSet.readString("FUPDate")));
            policy.setPremium(fieldSet.readInt("Premium"));
            policy.setNominee(fieldSet.readString("nominee"));
        } catch (Exception e) {
    ...
    private Date parseDate(String dateStr) throws BindException {
            ...
            {
                throw new BindException(dateStr ,"LicPolicy  BindException");
            }
